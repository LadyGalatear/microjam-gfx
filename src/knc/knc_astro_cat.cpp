...
namespace knc {
    ...
    knc_astro_cat::knc_astro_cat(int completed_games, const mj::game_data& data)
        : mj::game("knc"),
          ...
          _enemy1(bn::fixed_point(1000, 1000), _initial_speed(completed_games, data), 1), // parked off-screen
          ...
          _hit(false)
    {
        ...
        // start with three planets for EASY
        ...
        if (_difficulty == mj::difficulty_level::NORMAL ||
            _difficulty == mj::difficulty_level::HARD) {
    ...
    mj::game_result knc_astro_cat::play(const mj::game_data& data)
    {
        ...
        if (_difficulty == mj::difficulty_level::HARD) { // enemy only appears in hard mode
            _update_enemy(data, speed); // update enemy position and respawn if off-screen

            if (_enemy1.collides_with(_cat.position(), cat::COLLISION_RADIUS)) { // if enemy hits cat, end game
                _hit = true; // enemy is fast and deadly, so no need to check for victory condition - game ends immediately on hit
            }
        } 

        // end game if cat got hit
        return mj::game_result(_hit, false); // game ends immediately on hit, no need to check for victory condition here
    }
